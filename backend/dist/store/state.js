...
exports.networkState = exports.NetworkState = void 0;
...
class NetworkState extends events_1.EventEmitter {
    nodes;
    edges;
    alerts;
    metricsHistory;
    ...
    constructor() {
        ...
        this.nodes = JSON.parse(JSON.stringify(mockData_1.initialNodes));
    ...
    tickTelemetry() {
        ...
        this.nodes.forEach((node) => {
            ...
            if (node.cpu > 80)
                targetBaseTemp += 8; // hotter under load
            node.temp = Math.max(20, Math.min(85, node.temp + tempDelta + (node.temp < targetBaseTemp ? 1 : -1)));
            // Traffic (devices generate dynamic local traffic)
            const baseTraffic = node.type === 'router'
                ? 400
                : node.type === 'switch'
                    ? 200
                    : node.type === 'server'
        ...
        if (roll < 0.3) {
            ...
            if (nodeEdges.length) {
                ...
                message = `Зафиксирована повышенная задержка на канале ${randomNode.label} <-> ${randomEdge.source === randomNode.id
                    ...
                    : this.nodes.find((n) => n.id === randomEdge.source)?.label || 'Peer'}: ${Math.floor(Math.random() * 50) + 40} мс`;
...
exports.networkState = new NetworkState();
